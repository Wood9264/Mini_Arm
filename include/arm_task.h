...
const float JOINT1_JOYSTICK_ANGLE_STEP = JOINT1_MAX_ANGLE_PER_SECOND / 1000.0f * ARM_TASK_DELAY / JOYSTICK_MAX_OFFSET; // 每次代码执行时关节1的摇杆通道每单位的角度变化，度
const float JOINT2_JOYSTICK_ANGLE_STEP = JOINT2_MAX_ANGLE_PER_SECOND / 1000.0f * ARM_TASK_DELAY / JOYSTICK_MAX_OFFSET; // 每次代码执行时关节2的摇杆通道每单位的角度变化，度
const float JOINT3_JOYSTICK_ANGLE_STEP = JOINT3_MAX_ANGLE_PER_SECOND / 1000.0f * ARM_TASK_DELAY / JOYSTICK_MAX_OFFSET; // 每次代码执行时关节3的摇杆通道每单位的角度变化，度
const float JOINT4_JOYSTICK_ANGLE_STEP = JOINT4_MAX_ANGLE_PER_SECOND / 1000.0f * ARM_TASK_DELAY / JOYSTICK_MAX_OFFSET; // 每次代码执行时关节4的摇杆通道每单位的角度变化，度
const float JOINT5_JOYSTICK_ANGLE_STEP = JOINT5_MAX_ANGLE_PER_SECOND / 1000.0f * ARM_TASK_DELAY / BUTTON_MAX_OFFSET;   // 每次代码执行时关节5的按钮通道每单位的角度变化，度
const float TOOL_JOYSTICK_ANGLE_STEP = TOOL_MAX_ANGLE_PER_SECOND / 1000.0f * ARM_TASK_DELAY / TRIGGER_MAX_OFFSET;      // 每次代码执行时工具的触发通道每单位的角度变化，度
